        ...
        <div class="row">
            <div class="col-md-6 col-sm-12">
                <div class="input-group mb-3">
                    <div class="input-group-prepend">
                        <label class="input-group-text bg-light" for="extend">Category</label>
                    </div>
                    {{--<input type="text" name="Category" id="extend" class="form-control" placeholder="Please enter title" aria-label="Please enter title" aria-describedby="basic-addon2" value="{{ old('title') }}" />--}}
                    <select class="custom-select" name="category" id="extend" required>
                      @foreach(FOOD_CAT as $value => $caption)
                        <option value="{{ $value }}">{{ $caption }}</option>
                      @endforeach
                        {{--<option value="Main">Main Course</option>
                        <option value="Signatures">Darband Signatures</option>
                        <option value="Traditional">Traditional Dishes</option>
                        <option value="Stews">Stews with Rice</option>
                        <option value="Stews">Stews with Rice</option>
                        <option value="Appetizers">Cold Appetizers</option>
                        <option value="Appetizer">Hot Appetizers</option>--}}
                    </select>
                </div>
            </div>
            <div class="col-md-6 col-sm-12">
            ...
            <div class="col-md-3 col-sm-12">
                <div class="input-group mb-3">
                    ...
                    <input type="file" name="pic" id="pic" class="form-control" placeholder="Please enter video url" aria-label="Please enter video url" aria-describedby="basic-addon2" value="{{ old('pic') }}" required />
        ...
        <div class="col-12 table-responsive">
            <table class="table table-bordered">
                ...
                @foreach($dishes as $dish)
                  @if(isset(FOOD_CAT[$dish->extend]))
                    <tr>
                        <td>{{ FOOD_CAT[$dish->extend] }}</td>
                        <input type="hidden"  id="row_{{ $dish->id }}_cat" value="{{ $dish->extend }}" />
                        ...
                        <td class="operations pt-2">
                            ...
                            <a href="{{ url('boss/dishes/other/delete/' . $dish->id) }}" class="btn btn-danger my-1">
